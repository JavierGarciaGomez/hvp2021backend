...
const { createCollaborator, getCollaborators, getCollaboratorById, updateCollaborator, registerCollaborator, collaboratorLogin, collaboratorRenewToken, getCollaboratorsForWeb, deleteCollaborator, } = require("../controllers/collaboratorsController");
...
router.post("/create", [
    ...
    check("col_code", "El código de colaborador es obligatorio y debe contener tres letras").isLength({ min: 3, max: 3 }),
    validateJwt,
    fieldValidator,
], createCollaborator);
...
router.patch("/register", [
    ...
    check("col_code", "El código de colaborador es obligatorio y debe contener tres letras").isLength({ min: 3, max: 3 }),
    check("accessCode", "El código de acceso es obligatorio y debe contener 6 carácteres").isLength({ min: 6, max: 6 }),
    fieldValidator,
